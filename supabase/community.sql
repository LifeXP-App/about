-- GamiLife community board (Reddit-style: posts + upvotes + comments).
...
returns boolean language sql immutable as $$
  select
    ...
    or lower(trim(n)) like '%gamilife%'
    or lower(trim(n)) like '%gami life%'
...
-- ── Brand migration ──────────────────────────────────────────────────────────
-- Safe to re-run against an existing project after the LifeXP → GamiLife rename.
update public.community_posts
set author = 'GamiLife Team'
where is_official = true and lower(trim(author)) in ('lifexp team', 'life xp team');

update public.community_posts
set title = 'Using GamiLife to stop doomscrolling, replacing it with 20 min of reading'
where title = 'Using LifeXP to stop doomscrolling, replacing it with 20 min of reading';
...
values
  ('GamiLife Team', 'Welcome to the wall. What are you leveling up?',
  ...
  ('quietbuilder', 'Using GamiLife to stop doomscrolling, replacing it with 20 min of reading',
...
join (values
  ...
  ('Using GamiLife to stop doomscrolling, replacing it with 20 min of reading', 'bookwormbeck', 'Stealing this. 20 minutes feels doable.')
